      ...
      <div class="w-[800px]">
        <div class="flex flex-row border-gray-300">
          ...
          <div class="w-8 flex flex-col items-center gap-2 p-2 bg-gray-300 rounded-l-lg">
            <button @click="marginOnRight = !marginOnRight" title="Click increase/decrease the width of the editor">
              <IconChevronDoubleLeft v-if="!marginOnRight" />
              <IconChevronDoubleRight v-else />
            </button>
            <button @click="copyToClipBoard" title="Click to copy current text to the clipboard"
              class="active:animate-bounce transition duration-200 ease-in-out">
              <IconClipboard draggable="true" />
            </button>
            <div draggable="true" @dragstart.prevent="handleDragStart">
              <IconDrag />
            </div>
          </div>
          <!-- Text editor -->
          <div class="w-full" :class="marginOnRight ? 'mr-96' : ''">
            <textarea rows="30" name="plain_text" id="plain-text" v-model="plainText" :class="[
...
function handleDragStart() {
  ...
  window.electron.drag(plainText.value);
}
</script>